 ...
 *
 *  1. Article bodies are authored by admins, but "trusted author" is not a
 *     security model — a compromised admin account must not become stored XSS
 *     across every reader. This renderer ESCAPES first and only then emits a
 *     fixed set of tags, so there is no path from input to raw HTML at all.
  ...
  for (const rawLine of lines) {
    ...
    if (line.trim().startsWith("|")) {
      ...
      if (!inTable) {
        ...
        // sideways on a phone.
        out.push(
          '<div class="scroll-x my-5"><table class="w-full min-w-[24rem] border-collapse text-sm">',
        );
        out.push("<thead><tr>");
